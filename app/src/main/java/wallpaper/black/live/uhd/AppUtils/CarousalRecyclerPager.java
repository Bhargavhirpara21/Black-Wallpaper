...
public class CarousalRecyclerPager extends RecyclerView {
    public static final boolean DEBUG = BuildConfig.DEBUG;

    private CarousalRecyclerAdapter<?> mViewPagerAdapter;
    ...
    public CarousalRecyclerPager(Context context) {
        this(context, null);
    }

    public CarousalRecyclerPager(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CarousalRecyclerPager(Context context, AttributeSet attrs, int defStyle) {
    ...
    public Adapter getAdapter() {
        if (mViewPagerAdapter != null) {
            return mViewPagerAdapter.vhAdapter;
        }
        return null;
    }

    public CarousalRecyclerAdapter getWrapperAdapter() {
    ...
    public int getCurrentPosition() {
        int curPosition;
        if (getLayoutManager().canScrollHorizontally()) {
            curPosition = CarousalViewUtils.getCenterXChildPosition(this);
        } else {
            curPosition = CarousalViewUtils.getCenterYChildPosition(this);
        }
        if (curPosition < 0) {
    ...
    protected void adjustPositionX(int velocityX) {
        ...
        if (childCount > 0) {
            int curPosition = CarousalViewUtils.getCenterXChildPosition(this);
            ...
            if (targetPosition == curPosition
                ...
                View centerXChild = CarousalViewUtils.getCenterXChild(this);
    ...
    protected void adjustPositionY(int velocityY) {
        ...
        int childCount = getChildCount();
        if (childCount > 0) {
            int curPosition = CarousalViewUtils.getCenterYChildPosition(this);
            ...
            if (targetPosition == curPosition
                    && (!mSinglePageFling || mPositionOnTouchDown == curPosition)) {
                View centerYChild = CarousalViewUtils.getCenterYChild(this);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN && getLayoutManager() != null) {
            mPositionOnTouchDown = getLayoutManager().canScrollHorizontally()
                    ? CarousalViewUtils.getCenterXChildPosition(this)
                    : CarousalViewUtils.getCenterYChildPosition(this);
            if (DEBUG) {
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_DRAGGING) {
            mNeedAdjust = true;
            mCurView = getLayoutManager().canScrollHorizontally() ? CarousalViewUtils.getCenterXChild(this) :
                    CarousalViewUtils.getCenterYChild(this);
            if (mCurView != null) {
            ...
            if (mNeedAdjust) {
                int targetPosition = getLayoutManager().canScrollHorizontally() ? CarousalViewUtils.getCenterXChildPosition(this) :
                        CarousalViewUtils.getCenterYChildPosition(this);
                if (mCurView != null) {
    ...
    @NonNull
    protected CarousalRecyclerAdapter ensureRecyclerViewPagerAdapter(Adapter adapter) {
        if(adapter == null) {
            return null;
        }
        return (adapter instanceof CarousalRecyclerAdapter)
                ? (CarousalRecyclerAdapter) adapter
                : new CarousalRecyclerAdapter(this, adapter);
